...
export class MessageService {
    ...
    // ============================================
    // 1. ENVIAR MENSAJE (ACTUALIZADO CON STATUS)
    // ============================================
    ...
    // 6. MARCAR COMO LEÍDO 
    // ============================================
    async markAsRead(chatId: string, userId: string, messageId?: string): Promise<any[]> {
        const participant = await Participant.findOne({
            where: { chat_id: chatId, user_id: userId }
        });
        ...
        // Si se especifica un mensaje individual
        if (messageId) {
            try {
                await Message.update(
                    { 
                ...
                console.log('⚠️ Error al marcar mensaje como leído');
            }
        }

        //  Obtener los mensajes que se marcaron como leídos
    ...
    // ============================================
    // 14. ACTUALIZAR ESTADO DEL MENSAJE 
    // ============================================
    ...
    // ============================================
    // 15. OBTENER MENSAJES LEÍDOS 
    // ============================================
    async getReadMessages(chatId: string, userId: string): Promise<any[]> {
    ...
    private formatMessageResponse(message: any): MessageResponse {
        const response: MessageResponse = {
            ...
            status: message.status || 'sent',  // incluir status
